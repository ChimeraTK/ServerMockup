...
void variable::print() {
  if(dir == direction::IN) {
    std::cout << directory << "/" << name << " (read/write)"
              << "\t Desc: " << description << "\t unit: " << unit << "\t type: " << type
              << "\t number of elements: " << nElements << std::endl;
  }
  else {
    std::cout << directory << "/" << name << " (read only)"
              << "\t Directory: " << directory << "\t Desc: " << description << "\t unit: " << unit
              << "\t type: " << type << "\t number of elements: " << nElements << std::endl;
  }
}
  ...
  for(const auto& child : element.get_children()) {
    ...
    if(child->get_name() == "description") {
      try {
        ...
        std::cerr << "While reading description data:" << e.what() << std::endl;
        var.description = "";
      }
    }
      ...
      try {
        ...
        std::cerr << "While reading unit data:" << e.what() << std::endl;
        var.unit = "";
      }
    }
      ...
      else if(std::string(node->get_child_text()->get_content().c_str()).compare("Boolean") == 0) {
        var.type = ctk::DataType::Boolean;
      }
      else {
      ...
      if(type.compare("application_to_control_system") == 0) {
        var.dir = xml_parser::direction::OUT;
          ...
          type.compare("control_system_to_application_with_return") == 0) {
...
std::string xml_parser::getRootDir(std::string inputFile) {
  ...
  if(root->get_name() != "application") {
    throw xmlpp::exception("Expected root tag 'application' instead of: " + root->get_name());
  }
...
void xml_parser::addElement(const xmlpp::Element& element, ctk::Module* owner,
    ctk::TemplateUserTypeMapNoVoid<OutputList>* l1, ctk::TemplateUserTypeMapNoVoid<OutputArrayList>* l2,
    ctk::TemplateUserTypeMapNoVoid<InputList>* l3, ctk::TemplateUserTypeMapNoVoid<InputArrayList>* l4) {
  ...
  tmp.print();
#endif
  if(tmp.type == ctk::DataType::none || tmp.type == ctk::DataType::Void) return;
  if(tmp.nElements > 1) {
    if(tmp.dir == xml_parser::direction::OUT) {
      auto myLambda = [&](auto arg) {
        auto& accessorList = boost::fusion::at_key<decltype(arg)>(l2->table);
        accessorList.emplace_back(ctk::ArrayOutput<decltype(arg)>{owner, tmp.name, "", tmp.nElements, tmp.description});
      };
      ...
      auto myLambda = [&](auto arg) {
        ...
        accessorList.emplace_back(
            ctk::ArrayPollInput<decltype(arg)>{owner, tmp.name, "", tmp.nElements, tmp.description});
      };
      ctk::callForTypeNoVoid(tmp.type, myLambda);
    }
  }
    ...
    if(tmp.dir == xml_parser::direction::OUT) {
      auto myLambda = [&](auto arg) {
        ...
        accessorList.emplace_back(ctk::ScalarOutput<decltype(arg)>{owner, tmp.name, "", tmp.description});
      };
      ctk::callForTypeNoVoid(tmp.type, myLambda);
    }
    else {
      auto myLambda = [&](auto arg) {
        ...
        accessorList.emplace_back(ctk::ScalarPollInput<decltype(arg)>{owner, tmp.name, "", tmp.description});
      };
      ctk::callForTypeNoVoid(tmp.type, myLambda);
    }
  }
